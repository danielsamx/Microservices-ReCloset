  ...
  <Teleport to="body">
    <Transition name="modal">
      <div v-if="open"
        ...
        <div class="relative card-glass w-full sm:max-w-md flex flex-col modal-panel-enter-active shadow-card-hover
                    max-h-[85dvh] sm:max-h-[70vh] rounded-b-none sm:rounded-2xl">
          <header class="flex items-center gap-3 p-4 border-b shrink-0" style="border-color: var(--border-soft);">
            <span class="w-9 h-9 rounded-xl bg-gradient-to-br from-brand-300 to-brand-600 text-white grid place-items-center shrink-0 shadow-soft">
              <Icon name="bell" :size="18" />
            </span>
            <div class="min-w-0 flex-1">
              <h2 class="font-display font-bold text-base leading-tight">Notificaciones</h2>
              <p class="text-xs text-faint truncate">{{ notif.unread ? `${notif.unread} sin leer` : 'Sin pendientes' }}</p>
            </div>
            <button @click="emit('close')" type="button"
              class="w-10 h-10 grid place-items-center rounded-xl text-faint hover:text-brand-600 hover:bg-brand-500/10 transition shrink-0"
