...
async function load() {
  ...
  try { const { data } = await api.get('/catalog', { params: { per_page: 8 } }); items.value = data.data }
  catch (e) { error.value = true } finally { loading.value = false }
}
onMounted(load)
</script>
<template>
  <section class="relative overflow-hidden rounded-3xl bg-gradient-to-br from-brand-700 via-brand-600 to-teal-500 text-white px-6 sm:px-10 py-12 sm:py-16 shadow-card">
